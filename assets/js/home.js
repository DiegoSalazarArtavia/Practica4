...
document.addEventListener('click', (e) => {
    if (e.target && e.target.id === 'btnCalcular') {
        ...
        if (num1.toString().length == 0) {
            Swal.fire({ icon: 'error', title: 'Datos faltantes', text: 'Debe ingresar el número 1.', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
            document.getElementById("resultadoOperacion").textContent = "";
            return;
        }
        if (num2.toString().length == 0) {
            Swal.fire({ icon: 'error', title: 'Datos faltantes', text: 'Debe ingresar el número 2.', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
            document.getElementById("resultadoOperacion").textContent = "";
            return;
        }
...
document.addEventListener('click', (e) => {
    if (e.target && e.target.id === 'btnEvaluacion') {
        let nota = document.getElementById("nota").value;
        if (nota.toString().length == 0) {
            Swal.fire({ icon: 'error', title: 'Datos faltantes', text: 'Debe ingresar la nota.', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
            return;
        }
        let resultado = "";
